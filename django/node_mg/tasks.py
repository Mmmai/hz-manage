...
from .utils.cmdb_tools import update_asset_info, node_inventory, nodes_inventory
...
def sync_node_mg(self, model_id=None):
    ...
    try:
        ...
        for model in models_to_process:
            ...
            logger.info(
                f"模型 {model.name} 处理完成: 创建[{node_create_counter}],更新[{node_update_counter}]失败[{len(fail_instance)}],失败实例[{','.join(fail_instance)}]")
        ...
        logger.info(
            f"全部模型处理完成: 总创建[{total_node_create_counter}],总更新[{total_node_update_counter}]总失败[{total_fail_count}]")
...
def build_host_info(instance, ip):
    ...
    # elif instance.model.name == 'switches':
        ...
        return {
            ...
            'type': 2,
            'device_status': PublicModelInstanceService.get_instance_field_value(instance, 'device_status'),
...
def build_interfaces_and_args(host_info):
    ...
    type_map = {"agent": {"type": "1", "port": "10050"}, "snmp": {
        "type": "2", "port": "161"}, "ipmi": {"type": "3", "port": "623"}}
    interfaces = []
    other_args = {}
                ...
                if added:
                    for _added in added:
                        zabbix_api.create_interfaces(hostid=host_id, interface=_added,
                                                     template_names_by_type=template_names_by_type)
                if removed:
...
def zabbix_proxy_sync(self, proxy_info, action, node_ids=None):
    ...
    if action == 'delete':
        ...
        result = zabbix_api.update_proxy(proxyId=proxy_info.get("zbx_proxyid"),
                                         proxy_name=proxy_name, proxy_ip=proxy_ip)
        if result:
        ...
        if dissociate_hosts:
            ...
            if result:
                ...
                logger.error(
                    f"Failed to dissociate host {','.join(node_ips)} from Zabbix Proxy {proxy_name},response:{result}")
