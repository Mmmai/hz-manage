<template>
  <div class="card">
    <div class="table-header">
      ...
      <div class="header-button-ri">
        <el-select v-model="colValue" placeholder="Select" style="width: 120px">
          <el-option v-for="item in filterOptions" :key="item.value" :label="item.name" :value="item.value" />
        </el-select>
        <el-input v-model="filterValue" style="width: 240px" placeholder="回车查询" clearable @clear="getRules()"
          @keyup.enter.native="getRules()" />
        <!-- <el-button :icon="Refresh" circle @click="reloadWind" /> -->
      </div>
    </div>
    <el-table ref="multipleTableRef" :data="validationRules" style="width: 100%" border @sort-change="sortMethod">
      <el-table-column property="name" label="规则名" sortable="custom" />
      <el-table-column property="verbose_name" label="规则名称" />
      <el-table-column property="field_type" label="字段类型" sortable="custom" />
    ...
    <!-- 弹出框 -->
    <el-dialog v-model="dialogVisible" title="规则配置" width="500" :before-close="handleClose">
      <el-form :inline="true" label-position="right" :model="formInline" label-width="auto" ref="formRef">
        <el-form-item :label="item.label" :key="index" v-for="item, index in colLists" :required="item.required"
          :prop="item.value">
          <div v-if="item.value === 'rule'">
            <div v-if="formInline.field_type === 'enum'">
              <div v-for="item, index in tmpFormData" :key="index">
                <li>
                  <el-input v-model="item.name" style="width: 90px;margin-right: 10px;" />
                  <el-input v-model="item.value" style="width: 160px;margin-right: 10px;" />
            ...
            <el-input v-model="formInline[item.value]" type="textarea" clearable v-else style="width: 300px"
              :disabled="nowRow.built_in" />

          </div>

          <el-input v-model="formInline[item.value]" type="textarea" clearable v-else-if="item.value === 'description'"
            style="width: 300px" />



          <el-input v-model="formInline[item.value]" clearable v-else-if="item.value === 'name'"
            :disabled="nowRow.built_in || !isAdd ? true : false" />
          <el-input v-model="formInline[item.value]" clearable v-else-if="item.value === 'verbose_name'" />
          <div v-else>
            <el-select v-model="formInline.field_type" placeholder="Select" style="width: 120px"
              v-if="item.value === 'field_type'" :disabled="nowRow.built_in || !isAdd ? true : false">
              <el-option v-for="fItem in fieldOptions" :key="fItem.value" :label="fItem.label" :value="fItem.value" />
            </el-select>
            <el-select v-model="formInline.type" placeholder="Select" style="width: 120px" v-else
              :disabled="nowRow.built_in || !isAdd ? true : false">
              <el-option v-for="vItem in validate_type[formInline.field_type]" :key="vItem.type"
                :label="vItem.description" :value="vItem.type" />
            </el-select>
          </div>
        ...
        <div v-if="formInline.type === 'regex'" class="flexJstart">
          ...
          <el-input v-model="testRegex" clearable style="width: 200px;margin:0 10px;" />
          <el-icon style="color: var(--el-color-success) ;" :size="20" v-if="testRegexRes">
            <SuccessFilled />
          </el-icon>
...
import { ref, computed, watch, getCurrentInstance, onMounted, reactive, nextTick } from "vue";
...
const isUniq = computed(() => {
  return proxy.$commonFunc.hasDuplicates(Object.keys(arrayJson.value)) || proxy.$commonFunc.hasDuplicates(Object.values(arrayJson.value))
})
watch(() => arrayJson.value, (n) => {
...
const colLists = ref([
  { value: 'name', label: '规则名', sort: true, filter: true, type: 'string', required: true },
  { value: 'verbose_name', label: '规则名称', sort: false, filter: true, type: 'string', required: true },
  { value: 'field_type', label: '字段类型', sort: true, filter: true, type: 'object', required: true },
  { value: 'type', label: '规则类型', sort: true, filter: true, type: 'object' },
  { value: 'rule', label: '规则内容', sort: false, filter: true, type: 'string' },
  { value: 'description', label: '描述', sort: false, filter: false, type: 'text' },
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      ...
      if (isAdd.value) {
        ...
        let res = await proxy.$api.updateValidationRules({ id: nowRow.value.id, update_user: store.state.username, ...updateParams.value })
        console.log(res)
        // console.log(123)
        if (res.status == "200") {
