<template>
  <div style="display:flex;flex-direction: column;
    ...
    <div class="card" style="flex:0.3">
      <el-form :inline="true" :model="formInline" class="demo-form-inline" ref="formRef">
        <el-form-item label="数据源">
          <el-select v-model="formInline.dataSourceUrl" placeholder="Select" size="large" style="width: 180px">
        ...
        <div>
          <el-form-item label="过滤器" prop="labelValue">
            <div v-for="(value, key, index) in labelObject" :key="index">
              <el-select v-model="labelObject[key].labelName" filterable clearable placeholder="标签" style="width: 120px"
                @click="getLabels" @change="clearLabelValue(key)">
                <el-option v-for="labelItem in labelList" :key="labelItem.value" :label="labelItem.label"
                  :value="labelItem.value" />
              </el-select>
              <el-select v-model="labelObject[key].matchMethod" style="width: 65px" @change="clearLabelValue(key)">
                <el-option v-for="matchItem in matchOptions" :key="matchItem.value" :label="matchItem.label"
              ...
              <el-select filterable clearable reserve-keyword :loading="loading" remote-show-suffix
                v-model="labelObject[key].labelValue"
                :multiple="multipleList.includes(labelObject[key].matchMethod) ? false : true" style="width: 240px"
                remote @click="getLabelValue(key)" :remote-method="remoteSearch">
                <el-option v-for="labelOptionItem in labelObject[key].labelValueList" :key="labelOptionItem.value"
                  :label="labelOptionItem.label" :value="labelOptionItem.value" />
              </el-select>
              <el-button v-if="Object.keys(labelObject).length >> 1" size="small" :icon="Delete" circle
                style="margin: 0 5px 0px 5px;" @click="resetFilter(key)" />
            ...
            <el-button type="primary" :icon="CirclePlus" circle size="small" style="margin: 0 5px 0px 5px;"
              @click="addFilter" />
        ...
        <el-row justify="space-between">
          <el-col :span="12">
            <el-form-item label="条数">
              <el-input-number v-model="formInline.limit" :step="100" :min="1" :max="5000" width="20" />
            </el-form-item>

            <el-form-item label="关键字">
              <el-select v-model="formInline.matchKeyMethod" style="width: 60px">
                <el-option v-for="matchKeyItem in matchKeyOptions" :key="matchKeyItem.value" :label="matchKeyItem.label"
        ...
        ">
              ...
              <el-input v-model="formInline.matchKey" style="width: 240px" autosize type="textarea"
                placeholder="请输入关键字" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="时间范围" prop="dateValue" :rules="[{ required: true }]">
              <el-date-picker v-model="formInline.dateValue" type="datetimerange" :shortcuts="shortcuts"
                range-separator="To" start-placeholder="Start date" end-placeholder="End date" value-format="x"
    ...
    <!-- 文件导出 -->
    <div class="card">
    <el-table :data="resultLogs.queryResult" style="width: 98%" @expand-change="expandChange"
      ...
      <el-table-column label="日志等级" prop="level" width="100px" :filters="logLevelOption" :filter-method="filterTag"
      ...
      <el-table-column label="日志内容">
        <template #header>
          <div style="display:flex;justify-content: space-between;align-items: center;">
            <el-text style="color:var( --el-table-header-text-color )">日志内容</el-text>
          <!-- <el-input v-model="search" size="small" placeholder="Type to search" /> -->
          <el-button type="primary" plain v-if="isDownload" @click="downloadResLog(resultLogs?.queryResult)">下载日志</el-button>
          </div>
  ...
  <nearLogCom v-model:isShow="isShowNearLog" v-model:higlightKey="formInline.matchKey"
    v-model:url="formInline.dataSourceUrl" ref="childRef" />
...
import { ref, reactive, onMounted, watch, getCurrentInstance, computed, nextTick, onActivated } from 'vue'
...
const highlight = (content, keyword) => {
  ...
  if (!keyword) {
    ...
    const res = resultContent.replace(regex, `<span style="background-color: #FFFF00;">$&</span>`)
    return res;
  }
}
...
const remoteSearch = debounce((query) => {
  ...
  if (query === '' || query === undefined || query instanceof Object) {
    // console.log(111)
    labelObject[nowKey.value].labelValueList = labelObject[nowKey.value].initLabelValueList
    loading.value = false

  } else {
    labelObject[nowKey.value].labelValueList = labelObject[nowKey.value].initLabelValueList.filter(item => {
