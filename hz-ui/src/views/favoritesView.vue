  ...
  <div class="card">
    <el-scrollbar>
      <el-row justify="space-between">
        ...
        <!-- <el-col :span="1">
          <el-link type="primary" @click="isEdit == true">编辑</el-link></el-col
        > -->
        <el-col :span="1">
          <el-link type="primary" @click="gotoPortalUrl">添加</el-link></el-col
        >
      </el-row>
      ...
      <el-space
        wrap
        :size="30"
        v-draggable="[portalList, { animation: 150, onUpdate }]"
      >
        <el-card
            ...
            <el-space direction="vertical" alignment="end">
              <el-row>
                <el-col :span="8">
                  <el-avatar
                    :style="{
                      'background-color': getRandomColor(fItem.id + fItem.name),
                      'font-size': '28px',
                    }"
                    :size="60"
                  >
                ...
                <el-col :span="12">
                  <el-space direction="vertical">
                    <span
                      style="font-size: 20px; font-weight: bold; width: 200px"
                      >{{ fItem.name }}</span
                    >
                    <el-tooltip
                      class="box-item"
                      effect="light"
                      :content="fItem.describe"
                      placement="bottom"
                      v-if="fItem.describe === '' ? false : true"
                    >
                      <el-text class="describe-class" type="info">{{
                        fItem.describe
                      }}</el-text>
        ...
        <el-col :span="12">{{ fItem.group_name}}</el-col>
              ...
              <el-link
                type="info"
                @click.stop="setGroupFilter(fItem.group_name)"
                >{{ fItem.group_name }}</el-link
              >
            </el-space>
          </el-tooltip>
        </el-card>
  ...
  return portalList.value.filter((item) => {
    ...
    if (group.value === "所有") {
      ...
      if (filterPortal.value === "") {
        ...
        items = item.name
          .toLowerCase()
          .includes(filterPortal.value.toLowerCase());
      }
    } else {
