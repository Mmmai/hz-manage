<template>
  ...
  <div class="user-header">
    <div>
      <el-button  type="primary" size="small" @click="handleAddPortal">新增</el-button>
      <el-button  type="primary" size="small" @click="handleShowPgroup">新增分组</el-button>
      ...
      <el-dropdown size="small" split-button type="primary" style="margin-left: 15px;margin-right: 15px;">
      更多操作
      <template #dropdown>
        <el-dropdown-menu>
          <el-dropdown-item @click="exportData">全部导出</el-dropdown-item>
          <el-dropdown-item @click="exportDataSelect" v-if="multipleSelect.length === 0 " disabled >批量导出</el-dropdown-item>
          <el-dropdown-item @click="exportDataSelect" v-else>批量导出</el-dropdown-item>

          <el-dropdown-item>批量导入</el-dropdown-item>
          <el-dropdown-item @click="exportTemplate">模版下载</el-dropdown-item>
        </el-dropdown-menu>
      </template>
      </el-dropdown>
    </div>
    ...
    <el-form-item label="名称">
      <el-input v-model="filterObject.search" placeholder="输入名称" clearable />
    </el-form-item>
...
<!-- 门户组的弹出框 -->
  <el-dialog v-model="diaglogPgroup" title="编辑分组" width="30%">
      ...
      >
        ...
        >
          <el-form-item 
            ...
            :rules="[{required:true}]"
          >
            <el-input v-model="pgroupFormInline.group" placeholder="" clearable />
          </el-form-item>
          <el-row style="justify-content: space-around;">
            <el-form-item>
              <el-button @click="innerVisible = false">取消</el-button>
              <el-button type="primary" @click="handleCommitPgroup"> 确定</el-button>   
    ...
    <div class="user-header">
      <div>
        <el-button  type="primary" size="small" @click="handleAddPgroup">新增</el-button>
        <el-button  v-if="multipleSelect.length == 0" disabled type="danger" size="small" @click="handleDeleteMorePgroup">批量删除</el-button>
        <el-button v-else type="danger" size="small" @click="handleDeleteMorePgroup">批量删除</el-button>
      </div>
    </div>
    <el-table 
      :data="pgroupDataPage" 
      ...
      <el-table-column fixed="right" label="Operations" >
        <template #default="scope">
          <el-button  type="primary" size="small" @click="handleEditPgroup(scope.row)">
            编辑
          </el-button>
          
          <!-- <el-button v-if="scope.row.username == 'admin'" type="danger" size="small" disabled @click="handleDelete(scope.row)">删除</el-button> -->
          <el-button type="danger" size="small"  @click="handleDeletePgroup(scope.row)">
            删除
          </el-button>
      ...
      <el-row>
        <el-col :span="12">
          <el-form-item label="名称"  prop="name" :rules="[{required:true}]">
            <el-input v-model="portalFormInline.name" placeholder="" clearable />
          </el-form-item>
        </el-col>
      <el-col :span="12">
        <el-form-item label="链接地址" prop="url" :rules="[{required:true}]" style="width: 90%">
    ...
    <el-row>
      ...
      <el-col :span="12">
        <el-form-item label="分组" prop="group" :rules="[{required:true}]">
          <el-select v-model="portalFormInline.group" filterable placeholder="Select"               
            style="width: 120px"
            >
            <el-option
    ...
    <el-row>
      <el-col :span="12">
        <el-form-item label="用户名" prop="username">
          <el-input v-model="portalFormInline.username" placeholder="" clearable />
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item label="密码" prop="password" >
          <el-input v-model="portalFormInline.password" placeholder="" clearable />
        </el-form-item>
      </el-col>
    </el-row>
    <el-row>
...
const handleCurrentChangePgroup = (val) => {
  pgroupPageConfig.page = val
    ...
    pgroupDataPage.value = proxy.$commonFunc.pageFunc(pgroupData.value,pgroupPageConfig)
  }
// pgroup多选
...
const handleEditPgroup = (row) =>{
  ...
  proxy.$nextTick(() => {Object.assign(pgroupFormInline,row)})
...
const getPgroupData = async(config)=> {
  ...
  pgroupIsPage.value = pgroupTotal.value <= pgroupPageConfig.size ? true : false
...
const handleCommitPgroup = () => {
      proxy.$refs.pgroupForm.validate(async(valid) =>{
        ...
        // 编辑接口
            ...
            let res = await proxy.$api.pgroupUpdate({group:pgroupFormInline.group,id:pgroupFormInline.id})
            if (res.status == 200){
              innerVisible.value = false
...
const handleEdit = (row) =>{
  ...
  proxy.$nextTick(() => {Object.assign(portalFormInline,row)})
}
// 删除
...
const updatePartolStatus = async(param)=>{
  console.log(param)
  let res = await proxy.$api.portalUpdate({status:param.status,id:param.id})
  if (res.status == 200){
    ElMessage({
      type: 'success',
      message: '更新成功',
