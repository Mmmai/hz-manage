<template>
  ...
  <div class="card">
    <el-scrollbar>
      ...
      <el-row class="cimodelinfo" justify="space-between">
        <el-col :span="20">
          ...
          <el-space :size="30" style="width: 40%;">
            <el-button size="large" :icon="ciModelInfo.icon" circle disabled style=" margin: 10px;" />

            <!-- </div> -->

            <el-text>唯一标识: &nbsp;&nbsp;<el-text tag="b"> {{ ciModelInfo.name }}</el-text>
            </el-text>


            <el-text>名称:&nbsp;&nbsp; <el-text tag="b">{{ ciModelInfo.verbose_name }}</el-text> </el-text>

          </el-space>
        </el-col>
        <el-col :span="2" style="display: flex;align-items: center;justify-content: center;">
          <el-space alignment="flex-end">
            <el-button :disabled="ciModelInfo.built_in" icon="Delete" @click="deleteCiModel(ciModelInfo.id)" circle />
            <el-button :disabled="ciModelInfo.built_in" icon="Edit" @click="editCiModel(ciModelInfo)" circle />
      ...
      <el-tabs v-model="tabName" type="card" class="demo-tabs" @tab-click="handleClick">
        <el-tab-pane label="模型字段" name="field">
          <ciModelField ref="ciModelFieldRef" v-model:ciModelInfo="ciModelInfo" @getModelField="setModelField" />
        </el-tab-pane>
        <el-tab-pane label="唯一校验" name="verification">
          <ciModelUnique :modelId="modelId" :modelFieldLists="modelFieldLists" ref="ciModelUniqueRef" />
        </el-tab-pane>
  ...
  <el-dialog v-model="addModel" title="编辑模型" width="500" :before-close="handleClose">
    <el-form ref="modelFormRef" style="max-width: 600px" :model="modelForm" :rules="rules" label-width="auto"
      ...
      <el-form-item label="唯一标识" prop="name">
        <el-input v-model="modelForm.name" placeholder="请输入模型的唯一标识" disabled />
      </el-form-item>
      ...
      <el-form-item label="所属分组" prop="model_group">
        <el-select v-model="modelForm.model_group" placeholder="选择分组">
          <el-option :label="data.verbose_name" :value="data.id" :key="index" v-for="data, index in grouplist" />
        </el-select>
      </el-form-item>
    </el-form>
    <template #footer>
...
import { onMounted, reactive, getCurrentInstance, ref, toRefs, onActivated, watch } from "vue";
...
const modelCommit = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      let res = await proxy.$api.updateCiModel({ id: nowModelId.value, ...modelForm })
      console.log(res)
      // console.log(123)
      if (res.status == "200") {
