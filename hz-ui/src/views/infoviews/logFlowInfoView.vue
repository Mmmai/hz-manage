<template>
  ...
  >
    <template #extra>
      <el-tooltip content="切换流程是否换行显示" placement="top">
        <el-button type="primary" @click="isWrap ? isWrap = false : isWrap = true">切换</el-button>
      </el-tooltip>
    ...
    <el-descriptions-item>
      ...
      <el-tag  :type="missionStatus.level">{{ missionStatus.status }}</el-tag>
    </el-descriptions-item>
    <el-descriptions-item>
      <template #label>
        <div class="cell-item">
  ...
  <!-- direction="vertical" -->
      ...
      <el-space :wrap="isWrap" :size="10" v-loading="loading">
        <div 
          ...
          :key="index" 
          :style="{ width: '100%',heigth:'320px', display: 'flex','align-items': 'center' }"
        ...
        <el-card class="result-card" >
          ...
          <template #footer>
            <el-space :style="{ width: '100%', justifyContent: 'flex-end' }">
              <el-link type="primary" @click="downloadStepLog(v,allDataObj[v])" :disabled="allDataObj[v].queryResult.length === 0 ? true : false">下载日志</el-link>
              <el-link type="primary" @click="showStepLog(v,allDataObj[v])" :disabled="allDataObj[v].queryResult.length === 0 ? true : false">查看日志</el-link>
            </el-space>
          </template>
        </el-card>
        <el-icon v-if="Object.keys(allDataObj).length === index+1 ? false:true"><Right /></el-icon>
        </div>
      </el-space>
    </el-scrollbar>
  ...
  <el-drawer 
    ...
    <el-table-column label="日志内容"  prop="logLine"   >
      ...
      <div class="operation_show">
        <el-link type="primary" @click="clickToShowNearLog(row)">查看上下文
        </el-link>
  ...
  <nearLogCom v-model:isShow="isShowNearLog" v-model:higlightKey="matchKey" 
...
const hasLogModuleOptions = computed(()=>{
  ...
  flowSort.value.forEach(item => {
    console.log(item)
    if (allDataObj.value[item].status && allDataObj.value[item].queryResult.length >>> 0 ){
      var status = false
    }else{
      var status = true
    }
    tempObj.push({label:logModuleObj.value[item].module_name,value:item,disabled:status})
  });
  return tempObj
 })
...
const downloadStepLog = (param1,param2)=>{
  ...
  proxy.$commonFunc.downloadArray(`${module_name}-export-log-${nowTimeStr}`,exportArray)
...
onMounted(async () => {
  ...
  const resultTip = (params)=>{
    if (params.status){
      if (params.queryInfo.errorCount == 0 && params.queryResult.length >> 0){
        return '日志正常'
      }else if(params.queryInfo.errorCount >> 0 && params.queryResult.length >> 0){
        return '存在报错'
      }else{
        return '无请求信息'
      }
  ...
  const highlight = (content, keyword) => {
      ...
      const res = content.replace(regex,`<span style="background-color: #FFFF00;">$&</span>`)
      return res;
      }
  }
