<template>
  ...
  <div class="card">
    <el-dropdown style="margin-left: 15px;margin-right: 15px;" @command="handleCommand">
        ...
        <el-card style="width: 250px" shadow="hover" @click="edit(v)">
          <div class="el-card-context">
            <div style="display: flex;justify-content: space-between;align-content: center;">
              ...
              <img :src="getImgPath('loki_source.svg')" alt="Icon" style="width: 30px; height: 30px;"
                v-if="v.source_type == 'loki'" />
              <img :src="getImgPath('zabbix_source.svg')" alt="Icon" style="width: 30px; height: 30px;"
                v-else-if="v.source_type == 'zabbix'" />
              <span style="margin: 10px 0px 0px 10px;"> {{ v.source_name }}</span>
            </div>
  ...
  <!-- 弹出框 -->
  <el-dialog v-model="dialogVisible" :title="dataSourceType" width="500" :before-close="handleClose">
    <!-- <span @click=changeCom>This is a message</span> -->

    <component :is="componentId" v-model:formData=formLabelAlign ref="childRef"></component>
...
import { ref, reactive, shallowRef, getCurrentInstance, onMounted, defineAsyncComponent, computed } from 'vue'
...
//     // 在循环体中对当前数字进行操作
//     temp.push({label:'option'+i,value:i})
//   }
//   return temp
// })
...
//   else{
//     console.log(123)
//     options.value = initOptions.value.filter(item=>{
...
//     // 在循环体中对当前数字进行操作
//     initOptions.push({ label: 'option' + i, value: i })
//   }
...
//   return options.value.slice(0, (loadIndex.value + 1) * 20)
// });
...
// const loadOptions = debounce((query: string) => {
//   const filterOptions = initOptions.filter(o => o.label.includes(query));
//   options.value = filterOptions;
//   loadIndex.value = 0;
//   needShowMore.value = filterOptions.length > 20;
...
// const showMore = () => {
//   loadIndex.value += 1;
//   if (options.value.length === renderOptions.value.length) {
//     needShowMore.value = false;
//   }
// }
...
const updateAction = async () => {
  ...
  let res = await proxy.$api.dataSourceUpdate({ id: nowId.value, ...formLabelAlign })
  console.log(res)
  if (res.status == 200) {
    ElMessage({
      type: 'success',
