<template>
  <div class="card">
    ...
    <div class="flexCenter" style="width: 50%; margin-top: 10px" v-if="isEdit">
      <el-button @click="cancelAction()">取消</el-button>
      <el-button type="primary" @click="saveAction()">保存</el-button>
    </div>
    <div class="flexCenter" style="width: 50%; margin-top: 10px" v-else>
...
const hasIpField = computed(() => {
  return props.modelFieldLists.some((item) => item.name === "ip");
});
...
const saveAction = async () => {
  if (!hasIpField.value) {
    ElMessage({
      type: "error",
      message: `模型不存在名称为IP的模型字段，请先配置IP字段`,
    });
    return;
  }
  // 创建一个深拷贝的对象，避免修改原始数据
  const submitData = {
    ...formLine,
    zabbix_sync_info: formLine.zabbix_sync_info.filter((item) => item.type), // 过滤掉type为空的项
  };
