<template>
  ...
  <el-dialog v-model="multipleDia" title="批量更新" width="500" :before-close="handleClose">
    <el-form ref="multipleFormRef" style="max-width: 1000px" :model="multipleForm" label-width="auto">
      <el-form-item v-for="(pvalue, pkey, index) in multipleForm.updateParams" :key="pkey" :label="'字段' + (index + 1)"
        :prop="'updateParams.' + pkey" :rules="setUpdateFormItemRule(allModelFieldByNameObj[pkey]?.validation_rule)">
   ...
   :rules="setFormItemRule(item.name)"  -->
        <el-space>
          ...
          <el-select v-model="pkey" placeholder="选择字段" style="width: 120px">
            <el-option v-for="oitem in allModelFieldOptions" :key="oitem.value" :label="oitem.label"
              :value="oitem.value" />
          </el-select>
          <div v-if="pkey !== ''">
            <div v-if="['text', 'json'].indexOf(allModelFieldByNameObj[pkey].type) >>> -1 ? false : true">
              <el-input v-model="pvalue" type="textarea" autosize style="width: 180px;" />
            </div>
            <div v-else-if="['string'].indexOf(allModelFieldByNameObj[pkey].type) >>> -1 ? false : true">
              <el-input v-model="pvalue" style="width: 180px;" />
            </div>
            <div v-else-if="['enum'].indexOf(allModelFieldByNameObj[pkey].type) >>> -1 ? false : true">
              <el-select v-model="pvalue" placeholder="请选择" style="width: 180px;">
                <el-option v-for="ritem in enumOptionObj[allModelFieldByNameObj[pkey].validation_rule]"
                  :key="ritem.value" :label="ritem.label" :value="ritem.value" />
              </el-select>
            </div>
            <div v-else-if="['boolean'].indexOf(allModelFieldByNameObj[pkey].type) >>> -1 ? false : true">
              <el-switch v-model="pvalue" style="--el-switch-on-color: #13ce66; --el-switch-off-color: #ff4949;" />
            </div>
            <div v-else-if="['integer'].indexOf(allModelFieldByNameObj[pkey].type) >>> -1 ? false : true">
              <el-input-number v-model="pvalue" :step="1" style="width: 180px;" />

            </div>
            <div v-else-if="['float'].indexOf(allModelFieldByNameObj[pkey].type) >>> -1 ? false : true">
              <el-input-number v-model="pvalue" :precision="2" :step="0.1" style="width: 180px;" />
            </div>
            <div v-else-if="['date'].indexOf(allModelFieldByNameObj[pkey].type) >>> -1 ? false : true">
              <el-date-picker v-model="pvalue" type="date" placeholder="Pick a Date" format="YYYY-MM-DD"
                value-format="YYYY-MM-DD" style="width: 180px;" />
            </div>
            <div v-else-if="['datetime'].indexOf(allModelFieldByNameObj[pkey].type) >>> -1 ? false : true">
              <el-date-picker v-model="pvalue" type="datetime" placeholder="Pick a Date" format="YYYY/MM/DD hh:mm:ss"
                value-format="YYYY-MM-DD hh:mm:ss" style="width: 180px;" />
            </div>

            <div v-else>
              <el-input v-model="pvalue" />
    ...
    <template #footer>
      <div class="dialog-footer">
        ...
        <el-button type="primary" @click="saveCommit">保存</el-button>
      </div>
    </template>
  </el-dialog>
...
const setUpdateFormItemRule = (params) => {
  ...
  console.log(validationRulesObj.value[params].field_type)
...
const saveCommit = () => {
  ...
  console.log(JSON.stringify(modelFieldFormItemRule.value))
  console.log(modelFieldFormItemRule.value)
  console.log(multipleForm)
  console.log(multipleFormRef.value?.fields)
}
const options = ref([
  { value: '123', label: 'test' },
  { value: '456', label: 'test2' },
...
const allModelFieldOptions = computed<any>(() => {
  ...
  modelInfo.value?.field_groups?.forEach(item => {
    item.fields.forEach(field => {
      tempList.push({ value: field.name, label: field.verbose_name })
    });
  })
  return tempList
})
